...
function isUsername(name){
    if(name === '' || name.length < 6){
        getID('#iduserspan','Please re-enter username is valid !');
        flag = false;
    }else{ 
        getID('#iduserspan','');
        return true;
...
function isEmail(emailtxt){
    ...
    if(!emailtxt.match(emailregex)){
        getID('#idemailspan','Please re-enter email is valid !');
        flag = false;
    }else{ 
        getID('#idemailspan',''); 
        return true;
...
//    if(!password.match(passregex)){
    if(password.length < 8){
        getID('#idpassspan','Please re-enter password is valid !');
        flag = false;
    }else{ 
        getID('#idpassspan',''); 
        return true;
...
function isRe_pass(repass,bfpassword){
    ...
    if(count == 0){
        if(repass !==bfpassword ||repass===''){
            getID('#idre-passspan','Please re-enter re-password is valid !');
            flag = false;
            return false;
        }else{ 
            getID('#idre-passspan',''); 
